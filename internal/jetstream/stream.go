...
func (s *Stream) Subscribe(topicName string, handler am.MessageHandler[am.RawMessage], ops ...am.SubscriberOption) error {
	var err error

	subCfg := am.NewSubscriberConfig(ops)

	opts := []nats.SubOpt{
		nats.MaxDeliver(subCfg.MaxRetetionDeliver()),
	}
	cfg := &nats.ConsumerConfig{
		MaxDeliver: subCfg.MaxRetetionDeliver(),
	}
