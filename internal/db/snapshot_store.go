...
// use injected & configurable strategies
...
func (s SnapshotStore) Load(ctx context.Context, aggregate es.EventSourcedAggregate) (err error) {
...
func (s SnapshotStore) Save(ctx context.Context, aggregate es.EventSourcedAggregate) (err error) {
	const execute = `INSERT INTO %s (stream_id, stream_name, stream_version, snapshot_name, snapshot_data)
					VALUES ($1, $2, $3, $4, $5)
	...
	_, err = s.db.ExecContext(ctx, s.table(execute), aggregate.ID(), aggregate.AggregateName(), aggregate.PendingVersion(), snapshot.SnapshotName(), data)
