...
func NewEventStore(tableName string, db *sql.DB, reg registry.Registry) EventStore {
	return EventStore{
		tableName: tableName,
		db:        db,
		registry:  reg,
...
func (s EventStore) Save(ctx context.Context, aggregate es.EventSourcedAggregate) (err error) {
	const query = `INSERT INTO %s (stream_id, stream_name, stream_version, event_id, event_name, event_data, occurred_at)
	...
	for _, event := range aggregate.Events() {
		...
		if _, err = tx.ExecContext(ctx, s.table(query),
